...
class SharedBufferWorkletNode // eslint-disable-line no-unused-vars
  ...
  */
  constructor(context, options) {
    ...
    // this.fileSab = [new SharedArrayBuffer(44100*10*Int16Array.BYTES_PER_ELEMENT), new SharedArrayBuffer(44100*10*Int16Array.BYTES_PER_ELEMENT)];
    ...
    // filter.frequency.value = Math.floor(sampleRate / 2);

    // Worker backend.
    let _worker = new Worker('build/web/shared-buffer-worker.js');
    ...
    // let _worker2 = new Worker('build/web/shared-buffer-worker.js');
    // _worker2.onmessage = this._onWorkerInitialized.bind(this);
    // this._workers.push(_worker2);
    ...
    // if (this._workerOptions.channelCount == 2){
    //   c++;
    ...
    //   };
    //   if (this._workerOptions.directAction !== undefined){
    //     workerOptions2['directAction'] = 'directLoad';
    //   }
  
    //   _worker2.postMessage({
    //     message: 'INITIALIZE_WORKER',
    //     options: workerOptions2,
    //   });
  ...
  closeProcessor(){
    // this.port.postMessage({message : 'close'});
    // console.log('CLOSE PROCESSOR');
  ...
  processAudioKernel(States, MyConfig, InputRingBuffer, skipCounts, envelopes, arrMaxInt, SIZE, channelIdx, StatesDraw) {
  ...
  async loadAudioBuffer(wav, markers, sampleRate, sabDraw, channels, startPos = -1, endPos = -1){
    ...
    for (let c = 0 ; c < channelLength ; c++){
      ...
      // console.log( (new Date()).getTime() );
      this.processAudioKernel(States, MyConfig, arrRawInt , skipCounts, envelopes, arrMaxInt, SIZE, c, StatesDraw);
      // console.log( (new Date()).getTime() );
  ...
  */
  _onWorkerInitialized(eventFromWorker) {
    ...
    if (data.message === 'WORKER_READY') {
      ...
      console.log("data.channelIdx");
      console.log(data.SharedBuffers);
      // console.log(data.channelIdx);
      ...
      // wait until 2 channels
      // if (this.sabcs.length < this._workerOptions.channelCount){
      //   this.clearSamples(this.sabcs.length-1);
      //   return;
      // }
