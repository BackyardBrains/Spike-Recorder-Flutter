...
define(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    main() {
      var t1 = $.$get$_context();
      t1.$indexSet(0, "myHyperSuperMegaFunction", A.multiply___add$closure());
      t1.$indexSet(0, "multiplicationz", A.multiply__multiply$closure());
      t1.$indexSet(0, "enveloping", A.multiply__envelopingSamples$closure());
      t1.$indexSet(0, "unitInitializeEnvelope", A.multiply__unitInitializeEnvelope$closure());
      t1.$indexSet(0, "executeOneMessage", A.multiply__executeOneMessage$closure());
      t1.$indexSet(0, "executeContentOfMessageBuffer", A.multiply__executeContentOfMessageBuffer$closure());
      t1.$indexSet(0, "testEscapeSequence", A.multiply__testEscapeSequence$closure());
      t1.$indexSet(0, "frameHasAllBytes", A.multiply__frameHasAllBytes$closure());
      t1.$indexSet(0, "checkIfHaveWholeFrame", A.multiply__checkIfHaveWholeFrame$closure());
      t1.$indexSet(0, "areWeAtTheEndOfFrame", A.multiply__areWeAtTheEndOfFrame$closure());
      return 5567;
    },
    envelopingSamples(_head, sample, _envelopes, SIZE_LOGS2, skipCounts) {
      var t1, t2, t3, j, skipCount, interleavedSignalIdx, t4;
      A._asInt(_head);
      A._asInt(SIZE_LOGS2);
      for (t1 = J.getInterceptor$asx(_envelopes), t2 = J.getInterceptor$n(sample), t3 = J.getInterceptor$asx(skipCounts), j = 0; j < SIZE_LOGS2; ++j) {
        skipCount = A._asInt(t3.$index(skipCounts, j));
        interleavedSignalIdx = B.JSNumber_methods.floor$0(_head / skipCount) * 2;
        if (B.JSInt_methods.$mod(_head, skipCount) === 0) {
          J.$indexSet$ax(t1.$index(_envelopes, j), interleavedSignalIdx, sample);
          J.$indexSet$ax(t1.$index(_envelopes, j), interleavedSignalIdx + 1, sample);
        } else {
          if (t2.$lt(sample, J.$index$asx(t1.$index(_envelopes, j), interleavedSignalIdx)))
            J.$indexSet$ax(t1.$index(_envelopes, j), interleavedSignalIdx, sample);
          t4 = interleavedSignalIdx + 1;
          if (t2.$gt(sample, J.$index$asx(t1.$index(_envelopes, j), t4)))
            J.$indexSet$ax(t1.$index(_envelopes, j), t4, sample);
        }
      }
    },
    unitInitializeEnvelope(totalChannel, allEnvelopes, envelopeSizes, size, SIZE, SIZE_LOGS2) {
      var t1, t2, t3, t4, c, envelopes, i, sz, buffer, index;
      A._asInt(totalChannel);
      type$.List_List_List_double._as(allEnvelopes);
      type$.List_int._as(envelopeSizes);
      A._asDouble(size);
      A._asInt(SIZE);
      A._asInt(SIZE_LOGS2);
      for (t1 = J.getInterceptor$ax(allEnvelopes), t2 = type$.JSArray_double, t3 = J.getInterceptor$ax(envelopeSizes), t4 = type$.JSArray_List_double, c = 0; c < totalChannel; ++c) {
        envelopes = A._setArrayType([], t4);
        for (i = 0; i < SIZE_LOGS2; ++i) {
          sz = B.JSNumber_methods.ceil$0(size);
          if (B.JSInt_methods.$mod(sz, 2) === 1)
            ++sz;
          t3.add$1(envelopeSizes, sz);
          if (sz < 0)
            A.throwExpression(A.ArgumentError$("Length must be a non-negative integer: " + sz));
          buffer = A._setArrayType(new Array(sz), t2);
          for (index = 0; index < sz; ++index)
            buffer[index] = 0;
          B.JSArray_methods.add$1(envelopes, buffer);
          size /= 2;
        }
        t1.add$1(allEnvelopes, envelopes);
      }
    },
    executeOneMessage(typeOfMessage, valueOfMessage, offsetin) {
    },
    executeContentOfMessageBuffer(offset, messagesBuffer) {
      var t1, t2, stillProcessing, currentPositionInString, endOfMessage, startOfMessage, t3, k, arrStr, t4,
        message = new Uint8Array(64);
      for (t1 = J.getInterceptor$asx(messagesBuffer), t2 = type$.List_int, stillProcessing = true, currentPositionInString = 0, endOfMessage = 0, startOfMessage = 0; stillProcessing;) {
        if (J.$eq$(t1.$index(messagesBuffer, currentPositionInString), 59)) {
          for (t3 = endOfMessage - startOfMessage, k = 0; k < t3; ++k) {
            if (!(k < 64))
              return A.ioore(message, k);
            if (message[k] === 58) {
              t2._as(message);
              arrStr = B.Utf8Decoder_false.convert$1(message).split(":");
              t3 = arrStr.length;
              if (0 >= t3)
                return A.ioore(arrStr, 0);
              if (1 >= t3)
                return A.ioore(arrStr, 1);
              A._asString(offset);
              break;
            }
          }
          ...
          t3 = currentPositionInString - startOfMessage;
          t4 = A._asInt(t1.$index(messagesBuffer, currentPositionInString));
          if (!(t3 >= 0 && t3 < 64))
            return A.ioore(message, t3);
          message[t3] = t4;
          ++currentPositionInString;
          ++endOfMessage;
        }
        if (currentPositionInString >= 64)
          stillProcessing = false;
      }
    },
    testEscapeSequence(newByte, offset, messagesBuffer, weAreInsideEscapeSequence, messageBufferIndex, escapeSequenceDetectorIndex, oBuffHead) {
      var t1, i, cBufHead;
      if (A._asBool(weAreInsideEscapeSequence))
        if (J.$ge$n(messageBufferIndex, 64))
          A.executeContentOfMessageBuffer(offset, messagesBuffer);
        else {
          t1 = $.escapeSequence;
          A._asInt(escapeSequenceDetectorIndex);
          if (!(escapeSequenceDetectorIndex >= 0 && escapeSequenceDetectorIndex < 6))
            return A.ioore(t1, escapeSequenceDetectorIndex);
          if (t1[escapeSequenceDetectorIndex] === newByte)
            if (escapeSequenceDetectorIndex + 1 === 6)
              A.executeContentOfMessageBuffer(offset, messagesBuffer);
        }
      else {
        t1 = $.escapeSequence;
        ...
        if (t1[escapeSequenceDetectorIndex] === newByte)
          if (escapeSequenceDetectorIndex + 1 === 6) {
            for (t1 = J.getInterceptor$ax(messagesBuffer), i = 0; i < 64; ++i)
              t1.$indexSet(messagesBuffer, i, 0);
            for (cBufHead = 0, i = 0; i < 6; ++i) {
              --cBufHead;
              if (cBufHead < 0)
                cBufHead = 4095;
            }
            J.set$value$x(oBuffHead, cBufHead);
          }
      }
    },
    frameHasAllBytes(_numberOfChannels, circularBuffer, cBufTail) {
      var t1, t2, i,
        tempTail = A._asInt(J.$add$ansx(cBufTail, 1));
      if (tempTail >= 4096)
        tempTail = 0;
      for (t1 = J.getInterceptor$ns(_numberOfChannels), t2 = J.getInterceptor$asx(circularBuffer), i = 0; i < J.$sub$n(t1.$mul(_numberOfChannels, 2), 1); ++i) {
        if (A._asInt(J.$and$bn(t2.$index(circularBuffer, tempTail), 255)) > 127)
          return false;
        ++tempTail;
        if (tempTail >= 4096)
          tempTail = 0;
    ...
    checkIfHaveWholeFrame(circularBuffer, cBufTail, cBufHead) {
      ...
      if (tempTail >= 4096)
        tempTail = 0;
      for (t1 = J.getInterceptor$asx(circularBuffer); tempTail !== cBufHead;) {
        if (A._asInt(J.$and$bn(t1.$index(circularBuffer, tempTail), 255)) > 127)
          return true;
        ++tempTail;
        if (tempTail >= 4096)
          tempTail = 0;
    ...
    areWeAtTheEndOfFrame(circularBuffer, cBufTail) {
      var tempTail = A._asInt(J.$add$ansx(cBufTail, 1));
      if (A._asInt(J.$and$bn(J.$index$asx(circularBuffer, tempTail >= 4096 ? 0 : tempTail), 255)) > 127)
        return true;
      return false;
    }
  },
  ...
  A._Utf8Decoder.prototype = {
    convertGeneral$4(codeUnits, start, maybeEnd, single) {
      ...
      type$.List_int._as(codeUnits);
      end = A.RangeError_checkValidRange(start, maybeEnd, 64);
      if (start === end)
        return "";
      ...
      if ((t1 & 1) !== 0) {
        ...
        _this._state = 0;
        throw A.wrapException(new A.FormatException(message, codeUnits, _this._charOrIndex));
      }
      return result;
    },
  ...
  A.ButtonElement.prototype = {
    set$value(receiver, value) {
      receiver.value = A._asString(value);
    }
  };
  ...
  (function installTearOffs() {
    ...
    _static_1(A, "js___convertToJS$closure", "_convertToJS", 11);
    _static_1(A, "js___convertToDart$closure", "_convertToDart", 12);
    _static_2(A, "multiply___add$closure", "_add", 13);
    _static_2(A, "multiply__multiply$closure", "multiply", 14);
    _static(A, "multiply__envelopingSamples$closure", 5, null, ["call$5"], ["envelopingSamples"], 15, 0);
    _static(A, "multiply__unitInitializeEnvelope$closure", 6, null, ["call$6"], ["unitInitializeEnvelope"], 16, 0);
    _static(A, "multiply__executeOneMessage$closure", 3, null, ["call$3"], ["executeOneMessage"], 17, 0);
    _static_2(A, "multiply__executeContentOfMessageBuffer$closure", "executeContentOfMessageBuffer", 18);
    _static(A, "multiply__testEscapeSequence$closure", 7, null, ["call$7"], ["testEscapeSequence"], 19, 0);
    _static(A, "multiply__frameHasAllBytes$closure", 3, null, ["call$3"], ["frameHasAllBytes"], 2, 0);
    _static(A, "multiply__checkIfHaveWholeFrame$closure", 3, null, ["call$3"], ["checkIfHaveWholeFrame"], 2, 0);
    _static_2(A, "multiply__areWeAtTheEndOfFrame$closure", "areWeAtTheEndOfFrame", 20);
  })();
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A.Error, A.Iterable, A.ListIterator, A.FixedLengthListMixin, A.Symbol, A.MapView, A.ConstantMap, A.JSInvocationMirror, A.Closure, A._Required, A.MapMixin, A.LinkedHashMapCell, A.Rti, A._FunctionParameters, A.StreamTransformerBase, A.ListMixin, A._UnmodifiableMapMixin, A._Utf8Decoder, A.DateTime, A.OutOfMemoryError, A.FormatException, A.Null, A.StringBuffer, A.JsObject]);
    ...
    _inheritMany(A.Closure, [A.Closure2Args, A.Closure0Args, A.TearOffClosure, A.initHooks_closure, A.initHooks_closure1, A._convertToJS_closure, A._convertToJS_closure0, A._wrapToDart_closure, A._wrapToDart_closure0, A._wrapToDart_closure1]);
  ...
  var init = {
    ...
    mangledNames: {},
    types: ["@(@)", "@()", "bool(@,@,@)", "~(String,@)", "@(@,String)", "@(String)", "~(Object?,Object?)", "~(Symbol0,@)", "JsFunction(@)", "JsArray<@>(@)", "JsObject(@)", "Object?(Object?)", "Object?(@)", "int(int,int)", "double(double,double)", "@(int,@,@,int,@)", "@(int,List<List<List<double>>>,List<int>,double,int,int)", "~(@,@,@)", "~(@,@)", "~(@,@,@,@,@,@,@)", "bool(@,@)"],
    interceptorsByTag: null,
    leafTags: null,
    arrayRti: Symbol("$ti")
  };
  A._Universe_addRules(init.typeUniverse, JSON.parse('{"PlainJavaScriptObject":"LegacyJavaScriptObject","UnknownJavaScriptObject":"LegacyJavaScriptObject","JavaScriptFunction":"LegacyJavaScriptObject","AbortPaymentEvent":"Event","ExtendableEvent":"Event","MathMLElement":"Element","AudioElement":"HtmlElement","MediaElement":"HtmlElement","HtmlDocument":"Node","Document":"Node","DedicatedWorkerGlobalScope":"WorkerGlobalScope","CDataSection":"CharacterData","Text":"CharacterData","File":"Blob","NativeFloat32List":"NativeTypedArrayOfDouble","NativeByteData":"NativeTypedData","JSBool":{"bool":[]},"JSArray":{"List":["1"],"Iterable":["1"]},"JSUnmodifiableArray":{"JSArray":["1"],"List":["1"],"Iterable":["1"]},"JSNumber":{"double":[],"num":[]},"JSInt":{"double":[],"int":[],"num":[]},"JSNumNotInt":{"double":[],"num":[]},"JSString":{"String":[]},"EfficientLengthIterable":{"Iterable":["1"]},"ListIterable":{"Iterable":["1"]},"MappedListIterable":{"ListIterable":["2"],"Iterable":["2"],"ListIterable.E":"2"},"Symbol":{"Symbol0":[]},"ConstantMapView":{"UnmodifiableMapView":["1","2"],"_UnmodifiableMapView_MapView__UnmodifiableMapMixin":["1","2"],"MapView":["1","2"],"_UnmodifiableMapMixin":["1","2"],"Map":["1","2"]},"ConstantMap":{"Map":["1","2"]},"ConstantStringMap":{"ConstantMap":["1","2"],"Map":["1","2"]},"JSInvocationMirror":{"Invocation":[]},"Closure":{"Function":[]},"Closure0Args":{"Function":[]},"Closure2Args":{"Function":[]},"TearOffClosure":{"Function":[]},"StaticClosure":{"Function":[]},"BoundClosure":{"Function":[]},"JsLinkedHashMap":{"MapMixin":["1","2"],"Map":["1","2"]},"NativeTypedData":{"TypedData":[]},"NativeTypedArray":{"JavaScriptIndexingBehavior":["1"],"TypedData":[]},"NativeTypedArrayOfDouble":{"ListMixin":["double"],"JavaScriptIndexingBehavior":["double"],"List":["double"],"TypedData":[],"Iterable":["double"],"FixedLengthListMixin":["double"],"ListMixin.E":"double","FixedLengthListMixin.E":"double"},"NativeTypedArrayOfInt":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"]},"NativeInt16List":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeInt32List":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeInt8List":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeUint16List":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeUint32List":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeUint8ClampedList":{"ListMixin":["int"],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"NativeUint8List":{"ListMixin":["int"],"Uint8List":[],"JavaScriptIndexingBehavior":["int"],"List":["int"],"TypedData":[],"Iterable":["int"],"FixedLengthListMixin":["int"],"ListMixin.E":"int","FixedLengthListMixin.E":"int"},"MapBase":{"MapMixin":["1","2"],"Map":["1","2"]},"MapMixin":{"Map":["1","2"]},"MapView":{"Map":["1","2"]},"UnmodifiableMapView":{"_UnmodifiableMapView_MapView__UnmodifiableMapMixin":["1","2"],"MapView":["1","2"],"_UnmodifiableMapMixin":["1","2"],"Map":["1","2"]},"Utf8Decoder":{"Converter":["List<int>","String"]},"double":{"num":[]},"int":{"num":[]},"List":{"Iterable":["1"]},"HtmlElement":{"Node":[]},"AnchorElement":{"Node":[]},"AreaElement":{"Node":[]},"ButtonElement":{"Node":[]},"CharacterData":{"Node":[]},"DataElement":{"Node":[]},"Element":{"Node":[]},"FormElement":{"Node":[]},"InputElement":{"Node":[]},"LIElement":{"Node":[]},"MeterElement":{"Node":[]},"OptionElement":{"Node":[]},"OutputElement":{"Node":[]},"ParamElement":{"Node":[]},"ProgressElement":{"Node":[]},"SelectElement":{"Node":[]},"TextAreaElement":{"Node":[]},"_Attr":{"Node":[]},"JsArray":{"ListMixin":["1"],"List":["1"],"Iterable":["1"],"ListMixin.E":"1"}}'));
  A._Universe_addErasedTypes(init.typeUniverse, JSON.parse('{"EfficientLengthIterable":1,"NativeTypedArray":1,"StreamTransformerBase":2,"MapBase":2,"_JsArray_JsObject_ListMixin":1}'));
  var type$ = (function rtii() {
    ...
    return {
      ...
      List_List_List_double: findType("List<List<List<double>>>"),
...
;
  ...
  (function staticFields() {
    ...
    $.escapeSequence = A._setArrayType([255, 255, 1, 1, 129, 255], A.findType("JSArray<int>"));
  })();
