...
var onInit = function(sharedBuffers){
  ...
  .then( async (ports) => {
      ...
      if (this.isBoardIdentified === undefined) {
        ...
        let deviceIds = new Uint16Array(sharedBuffers.deviceId);
        deviceIds[0] = productId;
        deviceIds[1] = vendorId;
        ...
        sabDrawingState[DRAW_STATE.CHANNEL_COUNTS] = 1;
        ...
        await serialPort.open({baudRate: defaultBaudRate});
      }
...
self.onmessage = function( event ) {
  ...
  switch( event.data.command )
  {
    case "connect":
      ...
      if ( event.data.DEVICE_PRODUCTS !== undefined ){
        DEVICE_PRODUCT = event.data.DEVICE_PRODUCTS;
        const sabDeviceIdxInt = new Int16Array(event.data.sabDeviceIdx);
        if (sabDeviceIdxInt[0] >= 0){
          this.isBoardIdentified = true;
          this.strDevice = event.data.strDevice;  
        }else{
          this.isBoardIdentified = false;
          this.strDevice = event.data.strDevice;
