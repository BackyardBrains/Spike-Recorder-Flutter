...
public:
    ...
    ThresholdProcessor(int _channelCount, int sampleRate){
...
//     }    
    ...
    void process(short **outSamples, int *outSamplesCounts, short **inSamples,
        ...
        bool shouldReset = false;
        bool shouldResetLocalBuffer = false;
        int _selectedChannel = getSelectedChannel();
        // reset buffers if selected channel has changed
        if (lastSelectedChannel != _selectedChannel) {
            // debug_print("Resetting because channel has changed");

            // //__android_log_print(ANDROID_LOG_DEBUG, TAG, "Resetting because channel has changed");
            lastSelectedChannel = _selectedChannel;
            shouldReset = true;
        }
        // reset buffers if threshold changed
        if (lastTriggeredValue[_selectedChannel] != triggerValue[_selectedChannel]) {
            // debug_print("1");
            ...
            // debug_print("--------------");
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "Resetting because trigger value has changed");
            lastTriggeredValue[_selectedChannel] = triggerValue[_selectedChannel];
            shouldReset = true;
        }
        // reset buffers if averages sample count changed
        if (lastAveragedSampleCount != averagedSampleCount) {
        ...
        // let's save last channel count so we can use it to delete all the arrays
        int _channelCount = getChannelCount();

        int tmpLastChannelCount = lastChannelCount;
        if (lastChannelCount != _channelCount) {
            // debug_print("4");
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "Resetting because channel count has changed");
            lastChannelCount = _channelCount;
            shouldReset = true;
            shouldResetLocalBuffer = true;
        }
        if (shouldReset || resetOnNextBatch) {
        ...
        // for (c = 0; c < channelCount; c++) {
            ...
            for (i = 0; i < inSampleCounts[_selectedChannel]; i++) {
                currentSample = inSamples[_selectedChannel][i];
                ...
                */
                ...
                if (triggerType == -1) { //TRIGGER_ON_THRESHOLD // triggering by a threshold value
                    if (!inDeadPeriod) {
                        // check if we hit the threshold
                        if ((triggerValue[_selectedChannel] >= 0 && currentSample > triggerValue[_selectedChannel] &&
                                prevSample <= triggerValue[_selectedChannel]) ||
                            (triggerValue[_selectedChannel] < 0 && currentSample < triggerValue[_selectedChannel] &&
                                prevSample >= triggerValue[_selectedChannel])) {
                            // we hit the threshold, turn on dead period of 5ms
                            inDeadPeriod = true;
                            thresholdHit = true;
    ...
    void doClean(int _channelCount, bool resetLocalBuffer){
        clean(_channelCount, resetLocalBuffer);
    }
private:
    ...
    void init(bool resetLocalBuffer) {
        ...
        sampleCount = static_cast<int>(_sampleRate * timeSpanInt);
        bufferSampleCount = sampleCount / 2;

        if (resetLocalBuffer)buffer = new short *[_channelCount];
        samplesForCalculationCount = new int[_channelCount]{0};
        samplesForCalculationCounts = new int *[_channelCount];
        samplesForCalculation = new short **[_channelCount];
        summedSamplesCounts = new int *[_channelCount];
        summedSamples = new int *[_channelCount];
        averagedSamples = new short *[_channelCount];
        ...
        deadPeriodCount = static_cast<int>(_sampleRate * DEAD_PERIOD_SECONDS);
        deadPeriodSampleCounter = 0;
        inDeadPeriod = false;
        ...
        minBpmResetPeriodCount = (int) (_sampleRate * DEFAULT_MIN_BPM_RESET_PERIOD_SECONDS);
        lastTriggerSampleCounter = 0;
        sampleCounter = 0;
    }

    // Deletes all array fields used for calculations
    void clean(int _channelCount, bool resetLocalBuffer){
...
EXTERNC FUNCTION_ATTRIBUTE double createThresholdProcess(int _channelCount, uint32_t _sampleRate, short averagedSampleCount, short threshold){
    // highPassFilters = new HighPassFilter[channelCount];
    // count = (int) 2.0f * sampleRate;
    // debug_print("create1");
    sampleRate = (double) _sampleRate;

    channelCount = static_cast<short>(_channelCount);

    count = static_cast<int> (timeSpanInt * _sampleRate);
    ...
    for( int32_t i = 0; i < 1; i++ )
    {
        ...
        thresholdProcessor[i] = ThresholdProcessor( _channelCount, _sampleRate );
        ...
        thresholdProcessor[i].setChannelCount(_channelCount);
    ...
    const int NUMBER_OF_SEGMENTS = timeSpanInt;
    float SEGMENT_SIZE = static_cast<float>(_sampleRate);
    float SIZE = (NUMBER_OF_SEGMENTS * SEGMENT_SIZE);

    float size = SIZE * 4;
    for (short i = 0; i < SIZE_LOGS2; i++) {
        for (short j = 0 ; j < 6 ; j++){
            if (isAlreadyCreated == 1){
                // debug_print("delete envelopes?");
                // delete[] envelopes[j][i];
            }
            envelopes[j][i] = new short[static_cast<int>(size)];
        }
        size /= 2;
...
EXTERNC FUNCTION_ATTRIBUTE double initThresholdProcess(int _channelCount, double _sampleRate, double highCutOff, double q){
    for( int i = 0; i < _channelCount; i++ )
    {
...
EXTERNC FUNCTION_ATTRIBUTE double setTriggerTypeProcess(int channelIdx, int triggerType){
...
EXTERNC FUNCTION_ATTRIBUTE double setThresholdParametersProcess(int _channelCount, int _forceLevel, double _sampleRate, double _divider, int _current_start){
    channelCount = static_cast<short>(_channelCount);
    ...
    if (sampleRate != _sampleRate){
        ...
        const int NUMBER_OF_SEGMENTS = timeSpanInt;
        float SEGMENT_SIZE = static_cast<float>(sampleRate);
        float SIZE = (NUMBER_OF_SEGMENTS * SEGMENT_SIZE);
        ...
        for (short i = 0; i < SIZE_LOGS2; i++) {
            envelopeSizes[i] = (size);
            for (short j = 0 ; j < _channelCount ; j++){
                envelopes[j][i] = new short[static_cast<int>(size)];
            }
            size /= 2;
        }
...
int* nullData;
void resetEnvelope(int channelIdx, short **_envelopes, int _forceLevel){
    int sizeOfEnvelope = static_cast<int>(floor(2*envelopeSizes[_forceLevel]));
    for (int i = 0; i < sizeOfEnvelope; i++){
        _envelopes[_forceLevel][i] = 0;
    }

    // change me
    // std::fill(_envelopes[_forceLevel], _envelopes[_forceLevel] + sizeOfEnvelope, 0);
    // memset(envelopes[forceLevel],0,sizeOfEnvelope*sizeof(envelopes[forceLevel]));
}
...
void envelopingSamples(int _head, short sample, short **_envelopes, int _SIZE_LOGS2, int _forceLevel) {
    // for (int j = 0; j < SIZE_LOGS2; j++) {
        // if (forceLevel > -1 && j!=forceLevel) continue;
        int j = _forceLevel;
        int skipCount = skipCounts[j];
        int envelopeSampleIndex = static_cast<int>(floor(_head / skipCount));
        int interleavedSignalIdx = envelopeSampleIndex * 2;
...
*/
EXTERNC FUNCTION_ATTRIBUTE double getSamplesThresholdProcess(int channelIdx, short *data, int _forceLevel,double _divider, int currentStart, int sampleNeeded){

    divider = _divider;
    current_start = static_cast<int>(floor(currentStart));
    int sizeOfEnvelope = sampleNeeded;
    int rawSizeOfEnvelope = static_cast<int>(floor(sampleNeeded/2)) * skipCounts[_forceLevel];
    ...
    int maxEnvelopeSize = static_cast<int>(floor(envelopeSizes[0]/2));
    ...
    try{
        for( int32_t i = 0; i < channelCount; i++ )
        {
            if (i == channelIdx){
                ...
                for( int32_t jj = sampleStart; jj < sampleEnd; jj++ ){
                    envelopingSamples(j,tempSamplesPtr[i][jj], envelopes[i], SIZE_LOGS2, _forceLevel);
                    j++;
                }                
        ...
        std::copy(envelopes[channelIdx][_forceLevel], envelopes[channelIdx][_forceLevel] + sizeOfEnvelope, data);    
        resetEnvelope(channelIdx, envelopes[channelIdx], _forceLevel);
...
EXTERNC FUNCTION_ATTRIBUTE double appendSamplesThresholdProcess(short _averagedSampleCount, short _threshold, short _selectedChannel, short *data, uint32_t sampleCount, short *data2, uint32_t sampleCount2, short *data3, uint32_t sampleCount3, short *data4, uint32_t sampleCount4, short *data5, uint32_t sampleCount5, short *data6, uint32_t sampleCount6, short _channelCount, short _forceLevel, double _divider, int currentStart, int sampleNeeded, 
    ...
    if (_channelCount != channelCount){
        ...
        thresholdProcessor[0].setChannelCount(_channelCount);        
    }else{
        // channelCount = _channelCount;
    }
    forceLevel = _forceLevel;
    ...
    thresholdProcessor[0].setThreshold(static_cast<double>(_threshold));
    thresholdProcessor[0].setAveragedSampleCount(static_cast<double>(_averagedSampleCount));
    for (short c=0; c < _channelCount; c++){
        resetOutSamples(c, outSamplesPtr,outSampleCounts[c]);
    }
...
// ****
    ...
    if (_channelCount > 1) std::copy(data2, data2 + sampleCount2, inSamplesPtr[1]);
    if (_channelCount > 2) std::copy(data3, data3 + sampleCount3, inSamplesPtr[2]);
    if (_channelCount > 3) std::copy(data4, data4 + sampleCount4, inSamplesPtr[3]);
    if (_channelCount > 4) std::copy(data5, data5 + sampleCount5, inSamplesPtr[4]);
    if (_channelCount > 5) std::copy(data6, data6 + sampleCount6, inSamplesPtr[5]);
...
// ****
    ...
    thresholdProcessor[0].setSelectedChannel(_selectedChannel);
...
// ****
    ...
    int rawSizeOfEnvelope = static_cast<int>(floor(sampleNeeded * skipCounts[forceLevel] / 2)) ;
    ...
    // int sizeOfEnvelope = floor(envelopeSizes[forceLevel]/(divider/6));
    int sizeOfEnvelope = static_cast<int>(floor(sampleNeeded));
    for( short i = 0; i < _channelCount; i++ )
    {
        short channelIdx = i;
        std::copy(outSamplesPtr[channelIdx], outSamplesPtr[channelIdx] + maxEnvelopeSize, tempSamplesPtr[channelIdx]);
        // if (channelIdx == 1){
        ...
        // if (i == channelIdx){
            ...
            if (current_start != 0){
                ...
                // if (sampleEnd > maxEnvelopeSize){
                // }
                sampleStart = static_cast<int>(floor(maxEnvelopeSize / 2 - samplesLength / 2));
                sampleEnd = static_cast<int>(floor(maxEnvelopeSize / 2 + samplesLength / 2));
        ...
        for (int idx = 0; idx <= sizeOfEnvelope; idx++){
            envelopes[channelIdx][forceLevel][sizeOfEnvelope + idx] = 0;
        }
    ...
    if (_channelCount > 1) delete[] inSamplesPtr[1];
    if (_channelCount > 2) delete[] inSamplesPtr[2];
    if (_channelCount > 3) delete[] inSamplesPtr[3];
    if (_channelCount > 4) delete[] inSamplesPtr[4];
    if (_channelCount > 5) delete[] inSamplesPtr[5];
    return sizeOfEnvelope;
