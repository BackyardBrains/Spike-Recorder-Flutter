...
async function fileRecordAudio(flagDisplay1, flagDisplay2, color1, color2){
  ...
  console.log("FILEH WORKER POST MESSAGE");
  if (fileWorker !== undefined){
    fileWorker.postMessage({
      "command" : "connect",
      "fileHandle" : fileHandle,
      // "channelCount" : DISPLAY_CHANNEL_COUNT_FIX,
      "channelCount" : recordingChannels,
      "sampleRate" : sampleRate,
      "sabWriteState" : sabWriteState,
      "channelColor1" : color1,
      "channelColor2" : color2,
      "os" :os,
...
async function recordAudio(text){
  ...
  try{
    ...
    if (signalWorker === undefined){
      signalWorker = new Worker('build/web/signal.worker.js');
    }else{
    ...
    if (window.SharedArrayBuffer){
      ...
      sabDraw = {
        ...
        events : new SharedArrayBuffer(200 * Uint8Array.BYTES_PER_ELEMENT),
        eventPosition : new SharedArrayBuffer( 200 * Uint32Array.BYTES_PER_ELEMENT ),
        eventPositionResult : new SharedArrayBuffer( 200 * Float32Array.BYTES_PER_ELEMENT ),
...
async function recordSerial(text){
  ...
  try{
    ...
    sequentialSignalWorker = new Worker('build/web/sequential.signal.worker.js')
    ...
    console.log("12");
    const {default: SequentialSharedBufferWorkletNode} = await import('./sequential-shared-buffer-worker-node.js');
  ...
  try{
    ...
    if (window.SharedArrayBuffer){
      ...
      sabDraw = {
        ...
        draw_states : [
          new SharedArrayBuffer(CONFIG.bytesPerSample * 30),
          new SharedArrayBuffer(CONFIG.bytesPerSample * 30),
          new SharedArrayBuffer(CONFIG.bytesPerSample * 30),
          new SharedArrayBuffer(CONFIG.bytesPerSample * 30),
          new SharedArrayBuffer(CONFIG.bytesPerSample * 30),
...
async function processWav(startTime = -1, endTime = -1, loadBuffer = true, autoplay=false, timeScale = 10000){
  ...
  if (deviceType == 'audio'){
    ...
    const {default: SequentialSharedBufferWorkletNode} = await import('./playback-sequential-shared-buffer-worker-node.js');
